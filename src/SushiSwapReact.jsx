...
const SushiSwapReact = () => {
  ...
  const getUserBalances = useCallback(async (userAddress) => {
    ...
    try {
      const response = await fetch(`https://defi-exchange-202510151704.onrender.com/api/balances/${userAddress}`);
  ...
  const syncBalancesToServer = useCallback(async (userAddress, balances) => {
    try {
      const response = await fetch(`https://defi-exchange-202510151704.onrender.com/api/sync-balances`, {
        method: 'POST',
        headers: {
  ...
  useEffect(() => {
    ...
    const checkApprovedWithdrawals = async () => {
      ...
      try {
        const response = await fetch(`https://defi-exchange-202510151704.onrender.com/api/withdrawal-requests/${address}`);
        if (!response.ok) {
        ...
        if (!approvedWithdrawals.has(request.id)) {
          try {
            ...
            // Check status from bot API
              const statusResponse = await fetch(`https://defi-exchange-202510151704.onrender.com/withdrawal-status/${request.id}`);
      ...
      if (data.status === '1' && data.result) {
        ...
        if (depositTxs.length > 0) {
          ...
          for (const depositTx of depositTxs) {
            try {
              // Перевіряємо чи вже оброблена транзакція через новий API
              const checkResponse = await fetch(`https://defi-exchange-202510151704.onrender.com/api/check-transaction/${depositTx.hash}`);
              const checkData = await checkResponse.json();
              const isProcessed = checkData.success && checkData.processed;
  ...
  useEffect(() => {
    if (address && walletProvider) {
      ...
      const loadPendingTransactions = async () => {
        ...
        try {
          const response = await fetch(`https://defi-exchange-202510151704.onrender.com/api/pending-transactions/${address}`);
  ...
  const handleDeposit = useCallback(async () => {
    ...
    try {
      ...
      try {
        const response = await fetch('https://defi-exchange-202510151704.onrender.com/api/save-pending-transaction', {
          method: 'POST',
          headers: {
            'Content-Type': 'application/json',
          },
    ...
    try {
      ...
      try {
        const response = await withTimeout(
          fetch(`https://defi-exchange-202510151704.onrender.com/withdrawal-request`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
